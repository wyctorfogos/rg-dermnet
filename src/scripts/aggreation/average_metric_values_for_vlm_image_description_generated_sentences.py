...
if __name__ == "__main__":
    ...
    all_results = []
    for metadata_encoder_name  in ["gpt2","bert-base-uncased"]: # "one-hot-encoder"
        ...
        for vlm_sentece_generator in ["llava:34b", "qwen2.5:72b", "gemma3:27b"]:
            base_folder_path = f"./src/results/testes/generated-senteces-by-vlm-with-image-content-description/PAD-UFES-20"
            base_folder_path_by_metadata_encoder = base_folder_path+f"/textual-encoder-{metadata_encoder_name}"
            base_folder_path_by_model = base_folder_path_by_metadata_encoder+"/"+f"{vlm_sentece_generator}"
            for attention_mecanism in list_of_attention_mecanism:
                ...
                for model_name in list_of_models:
                    ...
                    try:
                        ...
                        result_df['vlm_sentence_generator'] = vlm_sentece_generator
            ...
            final_results_df = final_results_df[['attention_mecanism', 'model_name', 'textual_encoder', 'vlm_sentence_generator'] + [col for col in final_results_df.columns if col not in ['attention_mecanism', 'model_name', 'textual_encoder', 'vlm_sentence_generator']]]
    # Para todos os dados
    # Salvar os valores concatenados em um arquivo CSV
    final_results_df.to_csv(f'{base_folder_path}/all_metric_values_textual-encoder-all_encoders-vlm-sentences.csv', index=False)
    print(f"Todos os resultados foram salvos em {base_folder_path}/all_metric_values_textual-encoder-all_encoders-vlm-sentences.csv")
