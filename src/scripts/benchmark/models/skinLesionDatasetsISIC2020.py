...
class SkinLesionDataset(Dataset):
    def __init__(self, metadata_file, img_dir, drop_nan=False, bert_model_name='bert-base-uncased', size=(224, 224), is_train=False, random_undersampling=False, image_encoder="resnet-18", type_of_problem="binaryclass"):
        ...
        self.type_of_problem = type_of_problem # "binaryclass" # "binaryclass" # "multiclass" 
    ...
    def one_hot_encoding(self):
        ...
        if self.type_of_problem == "multiclass":
            ...
            # Apply mapping and filter out any rows with unmapped or NaN
            self.metadata['diagnosis'] = self.metadata['diagnosis'].map(valid_map)
            self.metadata = self.metadata.dropna(subset=['diagnosis']).reset_index(drop=True)
        dataset_features = self.metadata.drop(columns=['image_name', 'patient_id', 'diagnosis', 'benign_malignant', 'target'])
        ...
        # OneHotEncoder
        if os.path.exists(f"./src/results/preprocess_data/ohe_isic2020_{self.type_of_problem}.pickle"):
            with open(f'./src/results/preprocess_data/ohe_isic2020_{self.type_of_problem}.pickle', 'rb') as f:
                ohe = pickle.load(f)
            ...
            with open(f'./src/results/preprocess_data/ohe_isic2020_{self.type_of_problem}.pickle', 'wb') as f:
                pickle.dump(ohe, f)

        # StandardScaler
        if os.path.exists(f"./src/results/preprocess_data/scaler_isic2020_{self.type_of_problem}.pickle"):
            with open(f'./src/results/preprocess_data/scaler_isic2020.pickle_{self.type_of_problem}', 'rb') as f:
            ...
            with open(f'./src/results/preprocess_data/scaler_isic2020.pickle_{self.type_of_problem}', 'wb') as f:
        ...
        if self.type_of_problem=="binaryclass":
            labels = self.metadata['benign_malignant'].values
        else:
            labels = self.metadata['diagnosis'].values # ['nevus', 'melanoma', 'seborrheic keratosis', 'lentigo NOS', 'lichenoid keratosis', 'solar lentigo', 'cafe-au-lait macule', 'atypical melanocytic proliferation'] 

        if os.path.exists(f"./src/results/preprocess_data/label_encoder_isic2020_{self.type_of_problem}.pickle"):
            with open(f'./src/results/preprocess_data/label_encoder_isic2020_{self.type_of_problem}.pickle', 'rb') as f:
                label_encoder = pickle.load(f)
            encoded_labels = label_encoder.transform(labels)
        else:
            label_encoder = LabelEncoder()
            encoded_labels = label_encoder.fit_transform(labels)
            with open(f'./src/results/preprocess_data/label_encoder_isic2020_{self.type_of_problem}.pickle', 'wb') as f:
                pickle.dump(label_encoder, f)
        
        if self.type_of_problem=="binaryclass":
            return processed_data, encoded_labels, self.metadata['benign_malignant'].unique()
        
        return processed_data, encoded_labels, self.metadata['diagnosis'].unique()